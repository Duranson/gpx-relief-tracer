...
| 6. Contour lines | `generate_contour_segments`, `create_contour_object` | Marching-squares per cell per elevation level; result cached as `.npz` in `cache/` |
...
After step 7: materials applied, camera positioned, Cycles CPU render written to `render/render.png` (or `render/render_quick.png` when `QUICK_RENDER = True`).
...
`contour_cache_path` produces a SHA-256 digest of the selected DEM paths, GPX path, interval, and bounding box. Cache files are written as `cache/contours_cache_<digest>.npz`. Delete files in `cache/` to force regeneration.
