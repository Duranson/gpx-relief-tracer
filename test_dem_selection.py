...
def test_select_dem_collects_all_overlapping_tiles(tmp_path):
    ...
    gpx_x = np.array([5.0, 1015.0], dtype=np.float64)
    gpx_y = np.array([5.0, 1015.0], dtype=np.float64)

    selected = select_dem_tiles_for_gpx([dem_a, dem_b], gpx_x, gpx_y)

    assert selected == [dem_a, dem_b]
...
test_select_dem_collects_all_overlapping_tiles(tmp_folder)
